...
namespace ft
{
	...
	public :
		// constructor
		vector(void) : m_pos(NULL), m_size(0), m_cap(0) {}
		vector(const vector &origin) // 복사생성자
			: m_pos(NULL), m_size(origin.m_size), m_cap(origin.m_cap), m_alloc(origin.m_alloc)
		{
		...
		explicit vector(const allocator_type& _alloc)
			: m_pos(NULL), m_size(0), m_cap(0), m_alloc(_alloc) {}

		explicit vector(size_type _n)
			: m_pos(NULL), m_size(_n), m_cap(_n)
		{
			m_pos = m_alloc.allocate(m_size);
		}
